...
def populate_database( collection, input ):
    """
    ...
    try: 
        ...
        print( f"Successfully inserted data into collection: {collection}" )
    
    except Exception as e:
        print( f"Failed to insert into collection for reason: {e}" )


def get_data( site_string, number_of_rows=100000, pagination=1000 ):
    """
    ...
    json_string = ''
    for offset in range( 0, number_of_rows, pagination ):
        response = requests.get( site_string + '?$offset=' + str( offset ) )
